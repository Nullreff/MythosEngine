function Entity(name) {
	...
	if (this.health.current <= 0 && !this.dead) {
	    this.spawnPoint = this.type = "npc" 
		? map.getRandomLoc(true) 
		: map.spawn;
	    this.die();
	}
	...
	return Math.sqrt(Math.pow(this.location.x - entity.location.x, 2) + 
			 Math.pow(this.location.y - entity.location.y, 2));
    };
